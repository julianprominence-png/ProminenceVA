...
export default function AdminDashboard() {
  ...
  const fetchContacts = useCallback(async () => {
    ...
    try {
      ...
      const data: ContactRecord[] = snapshot.docs.map((doc) => ({
        id: doc.id,
        name: doc.data().name || "",
  ...
  const contactsToday = contacts.filter(
    (c) => c.createdAt && c.createdAt.toDate() >= todayStart
  ).length;
  const contactsWeek = contacts.filter(
    (c) => c.createdAt && c.createdAt.toDate() >= weekStart
  ).length;
  const contactsMonth = contacts.filter(
      ...
      <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-10">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="bg-gradient-to-b from-white/[0.08] to-transparent border border-white/10 border-b-black/60 shadow-[0_4px_10px_rgba(0,0,0,0.5),inset_0_1px_1px_rgba(255,255,255,0.3)] rounded-full px-5 py-2.5 text-[9px] font-bold tracking-[0.2em] uppercase text-white/70 hover:text-white hover:border-fuchsia-500/40 transition-all"
            >
              Refresh
            </button>
            <button
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-10">
          {[
            ...
            >
              ...
              />
              <p className="text-[9px] font-bold tracking-[0.25em] uppercase text-white/40 mb-3 relative z-10">
                {stat.label}
              </p>
              <p className="text-3xl md:text-4xl font-light tracking-tight relative z-10">
                {dataLoading ? (
                  <span className="inline-block w-10 h-8 bg-white/5 rounded-lg animate-pulse" />
                ) : (
                  stat.value
                )}
              </p>
          ...
          <div className="px-6 md:px-8 py-5 border-b border-white/[0.06] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-white/30 text-[10px] tracking-[0.15em] uppercase mt-1">
                {contacts.length} total transmissions received
              </p>
            </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/[0.05]">
                      <th className="text-left px-8 py-4 text-[9px] font-bold tracking-[0.25em] uppercase text-white/30">
                        Name
                      </th>
                    ...
                    {paginatedContacts.map((contact, idx) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-5">
                          <span className="text-sm text-fuchsia-300/80 font-light">
                            {contact.email}
                          </span>
                        </td>
                        <td className="px-6 py-5 max-w-xs">
                          <p className="text-sm text-white/60 font-light truncate">
                            {contact.message}
                          </p>
                        </td>
                        <td className="px-6 py-5">
        ...
        <footer className="mt-12 pb-8 text-center">
          <p className="text-white/20 text-[9px] tracking-[0.3em] uppercase font-bold">
            © {new Date().getFullYear()} Prominence Admin · Authorized Access
            Only
          </p>
        </footer>
      </div>
